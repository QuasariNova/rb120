...
class TaskManager
  ...
  # Because you initializes a local variable with the same name as the getter
  # for @tasks, Ruby will find the local variable first before it finds the
  # method. Because the local variable tasks hasn't been set, it has the value
  # of nil and nil does not have a #select method. By using self to call the
  # tasks method, we disambiguate from the local variable. It is probably better
  # to rename the local variable though, so that shadowing doesn't happen.
  def display_high_priority_tasks
    # tasks = task.select do |task|
