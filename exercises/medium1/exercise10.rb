...
# 2, 3, 4, 5) straights. For simplicity, your code only needs to recognize
# Ace-high straights.
...
class PokerHand
  ...
  def royal_flush?
    straight_flush? && hand.max.rank == Card::RANKS.last
  end
  ...
  def straight?
    hand.uniq { |card| card.rank }.size == 5 && hand.max - hand.min == 4
  end
  ...
  def two_pair?
    pairs = 0
    unique_cards = hand.uniq { |card| card.rank }
    unique_cards.each { |card| pairs += 1 if 2 == count_of(card.rank) }
    pairs == 2
  end
  ...
  def n_of_a_kind?(n)
    hand.each { |card| return true if n == count_of(card.rank) }
    false
  end
end
